...
public class ProductDAO implements Serializable {
    ...
    public int getAmountofProduct() throws Exception {
        ...
        try {
            ...
            String sql = "SELECT COUNT(*) AS AMOUNTOFPRODUCT "
                    + "FROM tblProduct "
                    + "WHERE Status = 1 ";
    ...
    public List<ProductDTO> loadAllProductWithPageNo(int pageNo, int itemPerPage) throws Exception {
        ...
        try {
            con = db.openConnection();
            String sql = "WITH LIST AS( SELECT ROW_NUMBER() OVER(ORDER BY createDate DESC) as STT ,* "
                    + "FROM tblProduct "
                    + "WHERE Status = 1 ) "
                    + "SELECT * "
    ...
    public List<ProductDTO> suggestProductByFirstProductID(String productID) throws Exception {
        ...
        try {
            con = db.openConnection();
            String sql = "Select P.ProductID, count(OD.OrderID), P.Name, P.Image, P.Price\n"
                    ...
                    + "					Where OD.ProductID = ? ) And P.Quantity > 0 And P.Status = 1\n"
                    + "Group By P.ProductID, P.Name, P.Image, P.Price\n"
                    + "Except\n"
                    + "Select P.ProductID, count(OD.OrderID), P.Name, P.Image, P.Price\n"
                    + "From tblOrderDetail OD Inner Join tblProduct P On OD.ProductID = P.ProductID \n"
                    + "Where P.ProductID = ? \n"
                    + "Group By P.ProductID, P.Name, P.Image, P.Price\n"
    ...
    public List<ProductDTO> suggestProductByUserID(String userID) throws Exception {
        list = new ArrayList<>();
        try {
            con = db.openConnection();
            String sql = "Select P.ProductID, count(P.ProductID) as Times ,P.Name, P.Image, P.Price\n"
                    + "From tblOrder O Inner Join tblOrderDetail OD On O.OrderID = OD.OrderID \n"
